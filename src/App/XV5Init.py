...
#
# Gathering all the information to start XV5
...
def InitApplications():
	try:
		...
		XV5.Console.PrintError("\n\nSeems the python standard libs are not installed, bailing out!\n\n")
		raise
	# Checking on XV5 module path ++++++++++++++++++++++++++++++++++++++++++
	ModDir = XV5.getHomePath()+'Mod'
	ModDir = os.path.realpath(ModDir)
	BinDir = XV5.getHomePath()+'bin'
	BinDir = os.path.realpath(BinDir)
	LibDir = XV5.getHomePath()+'lib'
	LibDir = os.path.realpath(LibDir)
	Lib64Dir = XV5.getHomePath()+'lib64'
	Lib64Dir = os.path.realpath(Lib64Dir)
	AddPath = XV5.ConfigGet("AdditionalModulePaths").split(";")
	HomeMod = XV5.ConfigGet("UserAppData")+"Mod"
	HomeMod = os.path.realpath(HomeMod)
	MacroDir = XV5.ParamGet("User parameter:BaseApp/Preferences/Macro").GetString("MacroPath")
	MacroMod = os.path.realpath(MacroDir+"/Mod")
	SystemWideMacroDir = XV5.getHomePath()+'Macro'
	SystemWideMacroDir = os.path.realpath(SystemWideMacroDir)

	#print XV5.getHomePath()
	if os.path.isdir(XV5.getHomePath()+'src\\Tools'):
		sys.path.append(XV5.getHomePath()+'src\\Tools')
	...
	if len(platform.mac_ver()[0]) > 0:
		sys.path.append(os.path.expanduser('~/Library/Application Support/XV5/Mod'))

# some often used shortcuts (for lazy people like me  ;-)
App = XV5
Log = XV5.Console.PrintLog
Msg = XV5.Console.PrintMessage
Err = XV5.Console.PrintError
Wrn = XV5.Console.PrintWarning
test_ascii = lambda s: all(ord(c) < 128 for c in s)

#store the cmake variales
App.__cmake__ = cmake;

Log ('Init: starting App::XV5Init.py\n')

# init every application by importing Init.py
InitApplications()

XV5.addImportType("XV5 document (*.FCStd)","XV5")
